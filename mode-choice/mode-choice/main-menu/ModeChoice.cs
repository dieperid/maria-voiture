...
namespace main_menu
{
    class ModeChoice : GameState
    {
        ...
        public ModeChoice(string name, StateHandler sh, AssetManager am, InputHandler ih, GameClock gc, RenderWindow w, WidgetHandler wh) : base(name, sh, am, ih, gc, w, wh)
        {
            _w = w;
        }
        ...
        public override void Load()
        {
            ...
            // Création du bouton de mode normal
            btnMultiplayer = _widgetHandler.CreateButton();
            btnMultiplayer.Texture = new Texture(Resource1.btnNormal); // Image 456 x 463
            btnMultiplayer.Position = new Vector2f(340, 350);
            btnMultiplayer.Size = new Vector2f(456, 463);
            btnMultiplayer.ClickedEvent += btnMultiplayerClick;
            btnMultiplayer.Over += btnOver;
            btnMultiplayer.NotOver += btnNotOver;


            //Création du bouton mode aléatoire
            btnSolo = _widgetHandler.CreateButton();
            btnSolo.Texture = new Texture(Resource1.btnAlea); // Image 456 x 463
            btnSolo.Position = new Vector2f(1028, 350);
            btnSolo.Size = new Vector2f(456, 463);
